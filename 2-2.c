...
int main(int argc, char *argv[])
{
    ...
    while(fgets(present, 100, inputFile))
    {
        ...
        if(currL <= currW && currL <= currH)
            {
                if(currW <= currH)
                    totalRibbon += 2*currL + 2*currW + currL*currW*currH;
                else if(currH <= currW)
                    totalRibbon += 2*currL + 2*currH + currL*currW*currH;
            }
        else if(currW <= currL && currW <= currH)
            {
                if(currL <= currH)
                    totalRibbon += 2*currW + 2*currL + currL*currW*currH;
                else if(currH <= currL)
                    totalRibbon += 2*currW + 2*currH + currL*currW*currH;
            }
        else if(currH <= currW && currH <= currL)
            {
                if(currW <= currL)
                    totalRibbon += 2*currH + 2*currW + currL*currW*currH;
                else if(currL <= currW)
                    totalRibbon += 2*currH + 2*currL + currL*currW*currH;
            }

        printf("%dx%dx%d: Total Paper = %d, Total Ribbon = %d\n", currL, currW, currH, totalSqFt, totalRibbon);
        currL = 0;
        currH = 0;
        currW = 0;
        x1posFound = false;
